...
public class DBAdapter
{
    ...
    public boolean editar_destacado_producto(String id_producto, int destacado)
    {
        ContentValues valores = new ContentValues();
        valores.put(CN_DESTACADO_PRODUCTO, destacado);
        String[] args = {id_producto};

        int n = db.update(TABLA_PRODUCTOS, valores, CN_ID_PRODUCTO + " =? ", args);

        if (n > 0)
        {
            Log.i(TAG, "Se actualizo el campo destacado del producto a " + destacado);
            return true;
        }
        else
        {
            Log.i(TAG, "No se pudo actualizar el campo destacado del producto.");
            return false;
        }
    }
